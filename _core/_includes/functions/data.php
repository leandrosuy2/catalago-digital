...
function delete_assinatura($id)
{
	...
	if (mysqli_query($db_con, "DELETE FROM assinaturas WHERE id = '$id'")) {
		...
		log_register($log_uid, $log_lid, $log_user_tipo . " " . $log_nome . " removeu Assinatura " . $nome . " às " . databr(date('Y-m-d H:i:s')));
	...
	function welcome_mail($email,$pass) {
		...
		if( html_mail( $email,"Bem vindo ao reidoscript!",$msg ) ) {
...
// cadastro simplificado
	...
	function edit_produto( $id,$estabelecimento,$categoria,$destaque,$estoque,$posicao,$ref,$nome,$descricao,$valor,$oferta,$valor_promocional,$variacao,$status,$visible,$integrado,$pesofrete,$alturafrete,$largurafrete,$comprimentofrete,$diametrofrete ) {
		...
		$updatedquery = "UPDATE produtos SET pesofrete=$pesofrete,alturafrete=$alturafrete,largurafrete=$largurafrete,comprimentofrete=$comprimentofrete,diametrofrete=$diametrofrete, rel_estabelecimentos_id = '$estabelecimento',rel_categorias_id = '$categoria',ref = '$ref',nome = '$nome',estoque = '$estoque',posicao = '$posicao',descricao = '$descricao',valor = '$valor',oferta = '$oferta',valor_promocional = '$valor_promocional',variacao = '$variacao',status = '$status',visible = '$visible',last_modified = '$datetime',integrado = '$integrado' WHERE id = '$id'";
