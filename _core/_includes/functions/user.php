...
function gera_comprovante($eid,$modo,$tamanho,$numero) {
    ...
    if(data_info('estabelecimentos',$eid,'calcularfrete')=='1'){
	    if($_SESSION['checkout']['forma_entrega']=='PAC04510'){
	        $frete_desc = 'PAC';
	        $frete_valor = $_SESSION['sacola']['frete']['04510']['valor'];
	    }
	    if($_SESSION['checkout']['forma_entrega']=='SEDEX04014'){
	        $frete_desc = 'SEDEX';
	        $frete_valor = $_SESSION['sacola']['frete']['04014']['valor'];
	    }
	...
	$comprovante .= "------\n\n";
	$comprovante .= "https://".$subdominiox.".veloximports.com.br\n";
...
function new_pedido( 
	...
	if( mysqli_query( $db_con, "INSERT INTO pedidos (
		...
		//geralink
		if($forma_pagamento == 99){
    	    $a = rand();
            $b = rand();
            $c = rand(5, 15);
            $reference = $a.$b.$c;
            $statuspagamento = 'pending';
            $access_token = data_info("estabelecimentos",$rel_estabelecimentos_id,"accesstoken");
            $estabelecimento = data_info("estabelecimentos",$rel_estabelecimentos_id,"nome");
            $url          = 'https://'.data_info("estabelecimentos",$rel_estabelecimentos_id,"subdominio").'.veloximports.com.br';
    	    $link = geralinkpagamento($vpedido,$peid,$estabelecimento,$access_token,$reference,$url);
    	    mysqli_query($db_con,"UPDATE pedidos SET linkpagamento='$link', referencia='$reference',statuspagamento='$statuspagamento' WHERE id='$peid'");
    	}
	    
		$comprovante = gera_comprovante($rel_estabelecimentos_id,"texto","1",$peid,false);
		$jsonpedido  = gera_comprovante($rel_estabelecimentos_id,"text","1",$peid,true);
        ...
        $pontosabater    = 0;
        foreach( $_SESSION['sacola'][$eid] AS $key => $value ) {
            $pontosadicionar = $_SESSION['sacola'][$eid][$key]['novos_pontos']+$pontosadicionar;
            $pontosabater    = $_SESSION['sacola'][$eid][$key]['pontos_abater']+$pontosabater;
            
    	}
    	$arraynovo = array(
    	    "pedido"=>$peid,
    	    "pontosadicionar"=>$pontosadicionar,
    	    "pontosabater"=>$pontosabater
    	);
    	
    	$qtdpontos = $_SESSION['qtdpontos'];
    	$query = mysqli_query($db_con,"SELECT * FROM clientes WHERE whatsapp = '$whatsapp'  AND id_estabelecimento = '$rel_estabelecimentos_id'");
    	
        $p256dh   = $_SESSION['p256dh'];
        $auth     = $_SESSION['auth'];
        $endpoint = $_SESSION['endpoint'];
        
    	if(mysqli_num_rows($query) > 0){
    	    $dados = mysqli_fetch_array($query);
    	    $pedidosanteriores = $dados['pontos_op'];
    	    $arrpontosop = array();
    	    if($pedidosanteriores != ''){
    	        $arrpontosop = array_values(json_decode($pedidosanteriores, true));
    	    }
    	    $arrpontosop[] = $arraynovo;
    	    $pontos_op = json_encode($arrpontosop);
    	    mysqli_query($db_con,"UPDATE clientes SET qtdpontos = '$qtdpontos',pontos_op='$pontos_op', qtdpedidos=qtdpedidos+1,p256dh='$p256dh',auth='$auth',endpoint='$endpoint' WHERE whatsapp = '$whatsapp'  AND id_estabelecimento = '$rel_estabelecimentos_id'");
    	    
    	}else{
    	    $arrpontosop = array();
    	    $arrpontosop[] = $arraynovo;
    	    $pontos_op = json_encode($arrpontosop);
    	    mysqli_query( $db_con, "INSERT INTO clientes(id_estabelecimento,nome,datadeinclusao,whatsapp,qtdpontos,pontos_op,ativo,p256dh,auth,endpoint) values('$rel_estabelecimentos_id','$nome',now(),'$whatsapp','$qtdpontos','$pontos_op',1,'$p256dh','$auth','$endpoint')");
    	}
    	
        if($licenca != ''){
		    mysqli_query( $db_con, "UPDATE pedidos SET comprovante = '$comprovante', json = '$jsonpedido' WHERE id = '$peid'" );
        }else{
		...
		//grava cliente e pontos
		$cidade = $estado;
		$estado = data_info("cidades",$cidade,"estado");
	    $query = mysqli_query($db_con,"SELECT * FROM clientes WHERE whatsapp = $whatsapp and id_estabelecimento = $rel_estabelecimentos_id");
	    if(mysqli_num_rows($query) > 0){
	        mysqli_query($db_con,"UPDATE clientes SET 
	                                                    cep = '$endereco_cep',
	                                                    rua = '$endereco_rua',
	                               ...
	                               WHERE whatsapp = $whatsapp AND id_estabelecimento = $rel_estabelecimentos_id ");
	    }else{
	        mysqli_query($db_con,"INSERT INTO clientes (id_estabelecimento, nome, datadeinclusao, whatsapp, ativo, cep, rua, numero, bairro, cidade, uf, complemento, referencia) 
	                       VALUES ('$rel_estabelecimentos_id', '$nome', now(), '$whatsapp', 1, '$endereco_cep', '$endereco_rua', '$endereco_numero', '$endereco_bairro', '$cidade', '$estado', '$endereco_complemento', '$endereco_referencia')"); 
	    }
...
function edit_pedido( $id,$status ) {
	...
	if( mysqli_query( $db_con, $updatedquery ) ) {
	    ...
	    $idapp = data_info('pedidos',$id,'rel_estabelecimentos_id');
	    $url   = 'https://'.data_info('estabelecimentos',$idapp,'subdominio').'.veloximports.com.br/pedidosabertos';
	    $nome  = data_info('estabelecimentos',$idapp,'nome');
	    
	    //dadosnotificação
	    $query = mysqli_query( $db_con, "SELECT * FROM pedidos WHERE id = '$id'");
    	$data  = mysqli_fetch_array($query);
    	
    	$whatsapp = $data['whatsapp'];
    	$eid      = $data['rel_estabelecimentos_id'];
        
        $query = mysqli_query( $db_con, "SELECT * FROM clientes WHERE id_estabelecimento = '$eid' AND whatsapp = '$whatsapp'");
        if(mysqli_num_rows($query) > 0){
    	    $data     = mysqli_fetch_array($query);
    	    $p256dh   = $data['p256dh'];
    	    $auth     = $data['auth'];
        ...
        if($status == 6){
            enviaNotificacao($nome,'🆗 Seu pedido está pronto para retirada.',$url,$p256dh,$auth,$endpoint);
        }
        if($status == 5){
            enviaNotificacao($nome,'🛵 Seu pedido está a caminho.',$url,$p256dh,$auth,$endpoint);
        }
        if($status == 4){
            enviaNotificacao($nome,'🆗 Seu pedido foi aceito.',$url,$p256dh,$auth,$endpoint);
        }
	    if($status == 3){
            enviaNotificacao($nome,'😢 Seu pedido foi cancelado. '.$mensagemPontos,$url,$p256dh,$auth,$endpoint);
	    }
	    if($status == 2){
            enviaNotificacao($nome,'🆗 Seu pedido foi concluído.'.$mensagemPontos.'Obrigado e volte sempre!.',$url,$p256dh,$auth,$endpoint);
	    }
		// SALVA LOG
...
function alterapontos($db_con,$id,$soma){
    ...
    if(mysqli_num_rows($query) > 0){
	    $data  = mysqli_fetch_array($query);
	    $pontos = json_decode($data['pontos_op']);
	    $newpontos = array();
	    for ($i=0; $i < count($pontos); $i++) {
            if($id==$pontos[$i]->pedido){
                if($soma==false){
                    $pontosdevolver = $pontos[$i]->pontosabater;
                }else{
                   $pontosdevolver = $pontos[$i]->pontosadicionar; 
                }
            }else{
                $newpontos[] = $pontos[$i];
            }
        }
        $novojson = json_encode($newpontos);
        $query = mysqli_query( $db_con, "UPDATE clientes SET qtdpontos=qtdpontos+$pontosdevolver,qtdpedidos=qtdpedidos-1, pontos_op='$novojson' WHERE id_estabelecimento = '$eid' AND whatsapp = '$whatsapp'");
    }
    return $pontosdevolver;
}
